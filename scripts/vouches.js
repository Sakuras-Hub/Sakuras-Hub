function renderVouchesMode() {
  try {
    ...
    fetch(VOUCHES_API_BASE + '?_=' + Date.now(), { headers: { 'Accept': 'application/json' } })
    ...
    fetch(VOUCHES_RAW + '?_=' + Date.now())
      ...
      .then(function(text) {
        ...
        if (jsonData && Array.isArray(jsonData) && jsonData.length) {
          renderFullVouchesFromJSON(panel, jsonData);
          return;
        }
        let bodyMatch = text.match(/<body>([\s\S]*?)<\/body>/);
        let bodyContent = bodyMatch ? bodyMatch[1] : '';

        let scriptMatch = text.match(/<script>([\s\S]*?)<\/script>/);
        ...
        jsCode = jsCode.replace(/document\.getElementById\('(vouch-list|search|total-count|img-count|lightbox|lb-img|lb-vid)'\)/g, "document.querySelector('#vouches-root #$1')");
        ...
        let vouchesHtml = '<div id="vouches-root">'
          ...
          + '  #vouches-root .vouch-card .attachments .file-link{display:inline-flex;align-items:center;gap:6px;padding:5px 10px;background:var(--bg2);border:1px solid var(--border);border-radius:6px;color:var(--accent);text-decoration:none;font-size:0.68rem;font-family:"Inter",sans-serif;}'
...
function renderMyVouches(data) {
  try {
    ...
    data.forEach(function(v) {
      ...
      var proofHtml = '';
      if (v.proof) {
        var pUrl = v.proof.indexOf('://') !== -1 ? v.proof : 'https://myvouch.es' + v.proof;
      ...
      var avatarUrl = v.platform_avatar ? (v.platform_avatar.indexOf('://') !== -1 ? v.platform_avatar : 'https://myvouch.es' + v.platform_avatar) : '';
...
function loadVouchesTabContent() {
  var root = document.getElementById('vouches-tab-root');
  if (!root) return;

  root.innerHTML = '<div class="panel-empty"><h3>Loading Vouches...</h3><p style="opacity:0.5">Fetching customer feedback</p></div>';

  var url = (typeof VOUCHES_RAW !== 'undefined') ? VOUCHES_RAW : 'https://raw.githubusercontent.com/Sakuras-Hub/Sakuras-Hub/refs/heads/main/need%20for%20the%20website%20to%20work/vouches.json';
  ...
  fetch(url + '?_=' + Date.now())
    ...
    .then(function(data) {
      if (!data || !data.length) {
        root.innerHTML = '<div class="panel-empty"><h3>No Vouches Yet</h3><p>Be the first to leave a vouch!</p></div>';
        return;
      }
      ...
      latest.forEach(function(v) {
        ...
        var proofHtml = '';
        if (v.proof_url) {
          if (v.proof_url.match(/\.(mp4|webm|mov)(\?|$)/i)) {
            proofHtml = '<a href="' + esc(v.proof_url) + '" target="_blank" rel="noopener"><video src="' + esc(v.proof_url) + '" preload="metadata" controlslist="nodownload" style="max-width:160px;max-height:120px;border-radius:6px"></video></a>';
          } else {
            proofHtml = '<a href="' + esc(v.proof_url) + '" target="_blank" rel="noopener"><img src="' + esc(v.proof_url) + '" loading="lazy" alt="Proof" style="max-width:160px;max-height:120px;border-radius:6px;object-fit:cover"></a>';
          }
        }
        var title = esc(v.buyer_name || 'Anonymous');
        var date = esc(v.date || '');
        var review = esc(v.review || '');
        cards += '<div class="vouch-card" style="background:rgba(17,15,28,0.82);border:1px solid var(--border);border-radius:13px;padding:14px;margin-bottom:10px">'
          + '<div class="meta" style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;flex-wrap:wrap;gap:6px">'
          + '<div class="author" style="font-size:0.82rem;font-weight:600;color:var(--text)">' + title + '</div>'
          + '<div class="date" style="font-size:0.6rem;color:var(--muted2);letter-spacing:0.5px;text-transform:uppercase">' + date + '</div>'
          + '</div>'
          + '<div class="content" style="font-size:0.8rem;line-height:1.55;color:var(--text);white-space:pre-wrap;margin-bottom:8px">' + starsHtml + '\n' + review + '</div>'
          + (proofHtml ? '<div class="attachments" style="display:flex;gap:8px;flex-wrap:wrap">' + proofHtml + '</div>' : '')
          + '</div>';
      });

      root.innerHTML = '<div style="padding:4px 0">'
        + '<div class="stats" style="display:flex;gap:12px;justify-content:center;flex-wrap:wrap;padding:6px 0 10px">'
        + '<span style="background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);padding:5px 12px;font-size:0.68rem;color:var(--muted);letter-spacing:1px">Total <b style="color:var(--accent)">' + data.length + '</b></span>'
        + '<span style="background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);padding:5px 12px;font-size:0.68rem;color:var(--muted);letter-spacing:1px">Showing latest <b style="color:var(--accent)">' + latest.length + '</b></span>'
        + '</div>'
        + '<div style="font-size:0.65rem;text-align:center;opacity:0.4;margin-bottom:8px;letter-spacing:1px;text-transform:uppercase">Latest ' + latest.length + ' Vouches</div>'
        + '<div class="vouch-list">' + cards + '</div>'
        + '</div>';
    })
    .catch(function(err) {
      root.innerHTML = '<div class="panel-empty"><h3>\u26a0\ufe0f Could not load Vouches</h3><p style="color:#ff6677;font-size:0.82rem">' + esc(err.message || String(err)) + '</p></div>';
    });
}

function renderFullVouchesFromJSON(panel, data) {
  try {
    if (!data || !data.length) {
      panel.innerHTML = '<div class="panel active"><div class="panel-empty"><h3>No Vouches Yet</h3></div></div>';
      return;
    }
    var total = data.length;
    var stars5 = 0, stars4 = 0;
    ...
    data.forEach(function(v) {
      ...
      if (v.proof_url) {
        if (v.proof_url.match(/\.(mp4|webm|mov)(\?|$)/i)) {
          proofHtml = '<a href="' + esc(v.proof_url) + '" target="_blank" rel="noopener"><video src="' + esc(v.proof_url) + '" preload="metadata" controlslist="nodownload" style="max-width:160px;max-height:120px;border-radius:6px;object-fit:cover"></video></a>';
        } else {
          proofHtml = '<a href="' + esc(v.proof_url) + '" target="_blank" rel="noopener"><img src="' + esc(v.proof_url) + '" loading="lazy" alt="Proof" style="max-width:160px;max-height:120px;border-radius:6px;object-fit:cover"></a>';
        }
      }
      var name = esc(v.buyer_name || 'Anonymous');
      var date = esc(v.date || '');
      var review = esc(v.review || '');
      cards += '<div class="vouch-card">'
        + '<div class="meta"><div class="author">' + name + '</div><div class="date">' + date + '</div></div>'
        + '<div class="content">' + starsHtml + '\n' + review + '</div>'
        + (proofHtml ? '<div class="attachments">' + proofHtml + '</div>' : '')
        + '</div>';
    });
    var vhtml = '<div id="vouches-root">'
      + '<style>'
      + '#vouches-root .stats{display:flex;gap:12px;justify-content:center;flex-wrap:wrap;padding:6px 0 10px;}'
      + '#vouches-root .stats span{background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);padding:5px 12px;font-size:0.68rem;color:var(--muted);letter-spacing:1px}'
      + '#vouches-root .stats b{color:var(--accent)}'
      + '#vouches-root .vouch-card{background:rgba(17,15,28,0.82);border:1px solid var(--border);border-radius:13px;padding:14px;margin-bottom:10px}'
      + '#vouches-root .vouch-card:hover{border-color:rgba(255,0,255,0.30)}'
      + '#vouches-root .vouch-card .meta{display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;flex-wrap:wrap;gap:6px}'
      + '#vouches-root .vouch-card .author{font-size:0.82rem;font-weight:600;color:var(--text)}'
      + '#vouches-root .vouch-card .date{font-size:0.6rem;color:var(--muted2);letter-spacing:0.5px;text-transform:uppercase}'
      ...
      + '#vouches-root .vouch-card .attachments a:hover{border-color:var(--accent);box-shadow:0 0 14px var(--accent-glow2)}'
      + '#vouches-root .vouch-card .attachments img,#vouches-root .vouch-card .attachments video{width:100%;display:block;max-height:140px;object-fit:cover;border-radius:4px}'
      + '#vouches-root .vouch-card .attachments .file-link{display:inline-flex;align-items:center;gap:6px;padding:5px 10px;background:var(--bg2);border:1px solid var(--border);border-radius:6px;color:var(--accent);text-decoration:none;font-size:0.68rem}'
      + '@media(max-width:640px){#vouches-root .vouch-card{padding:12px}#vouches-root .vouch-card .content{font-size:0.75rem}#vouches-root .vouch-card .author{font-size:0.78rem}#vouches-root .stats span{font-size:0.6rem;padding:4px 8px}#vouches-root .vouch-card .attachments a{max-width:100%}}'
      + '</style>'
      + '<div class="stats"><span>Total <b>' + total + '</b></span><span>\u2b50 5\u2605 <b>' + stars5 + '</b></span><span>\u2b50 4\u2605 <b>' + stars4 + '</b></span></div>'
      + '<div class="vouch-list">' + cards + '</div>'
      + '</div>';
    panel.innerHTML = '<div class="panel active" style="padding:0;max-width:100%;">' + vhtml + '</div>';
    updateHero("Sakura's Vouches", 'Customer Feedback');
  } catch(e) {
    panel.innerHTML = '<div class="panel active"><div class="panel-empty"><h3>\u26a0\ufe0f Error</h3><p style="color:#ff6677">' + esc(e.message || String(e)) + '</p></div></div>';
  }
}
